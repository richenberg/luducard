...
luducard-err-emulator-path-missing = Эта папка эмулятора больше не существует.
luducard-err-no-emulator-detected = В этой папке не найден поддерживаемый эмулятор. Проверьте, есть ли там исполняемый файл эмулятора.
luducard-err-nothing-to-export = Нет файлов для экспорта.
luducard-err-no-save-path-to-open = У этой игры пока нет папки сохранений. Сначала выполните сканирование.
luducard-err-no-crash-safety-backup = Для этой игры нет аварийного снимка, поэтому откатывать нечего.
luducard-err-active-profile-delete = Активный профиль сохранений нельзя удалить. Сначала переключитесь на другой.
luducard-err-exe-folder-not-writable = В папку приложения нельзя записывать. Проверьте права или переместите Luducard туда, где запись разрешена.
luducard-err-invalid-luducard-file = Это не корректный файл .luducard: { $message }
luducard-err-preset-metadata-corrupt = Метаданные этого пресета повреждены: { $message }
luducard-err-preset-incomplete = Этот локальный пресет неполный или повреждён.
luducard-err-install-dir-not-found = Не удалось найти папку установки «{ $game }».
luducard-err-save-file-locked = Не удалось заменить прежний файл сохранения { $path }: { $message }. Закройте игру и попробуйте снова.
