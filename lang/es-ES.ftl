...
luducard-err-emulator-path-missing = Esa carpeta del emulador ya no existe.
luducard-err-no-emulator-detected = No se encontró ningún emulador compatible en esta carpeta. Comprueba que el ejecutable del emulador esté ahí.
luducard-err-nothing-to-export = No hay archivos que exportar.
luducard-err-no-save-path-to-open = Este juego aún no tiene carpeta de partidas que abrir. Haz un análisis primero.
luducard-err-no-crash-safety-backup = No se encontró ninguna copia de seguridad automática para este juego, así que no hay nada que revertir.
luducard-err-active-profile-delete = No se puede eliminar el perfil de partidas activo. Cambia a otro perfil primero.
luducard-err-exe-folder-not-writable = No se puede escribir en la carpeta de la aplicación. Revisa los permisos o mueve Luducard a una ubicación donde puedas escribir.
luducard-err-invalid-luducard-file = Este no es un archivo .luducard válido: { $message }
luducard-err-preset-metadata-corrupt = Los metadatos de este preset están corruptos: { $message }
luducard-err-preset-incomplete = Este preset local está incompleto o no es válido.
luducard-err-install-dir-not-found = No se pudo encontrar la carpeta de instalación de "{ $game }".
luducard-err-save-file-locked = No se pudo reemplazar el archivo de partida anterior { $path }: { $message }. Cierra el juego e inténtalo de nuevo.
