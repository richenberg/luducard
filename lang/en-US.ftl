...
luducard-err-emulator-path-missing = That emulator folder no longer exists.
luducard-err-no-emulator-detected = No supported emulator found in this folder. Check that the emulator's executable is there.
luducard-err-nothing-to-export = There are no files to export.
luducard-err-no-save-path-to-open = This game has no save folder to open yet. Run a scan first.
luducard-err-no-crash-safety-backup = No crash-safety snapshot was found for this game, so there is nothing to roll back to.
luducard-err-active-profile-delete = The active save profile cannot be deleted. Switch to another profile first.
luducard-err-exe-folder-not-writable = The app's folder is not writable. Check its permissions, or move Luducard somewhere you can write to.
luducard-err-invalid-luducard-file = This is not a valid .luducard file: { $message }
luducard-err-preset-metadata-corrupt = This preset's metadata is corrupted: { $message }
luducard-err-preset-incomplete = This local preset is incomplete or invalid.
luducard-err-install-dir-not-found = Could not find the install folder for "{ $game }".
luducard-err-save-file-locked = Could not replace the previous save file { $path }: { $message }. Close the game and try again.
luducard-backup-kind-quick = Quick-save
luducard-add-version-note = Add a note...
luducard-versioning = Save Versioning
luducard-versioning-desc = Keep a history of previous saves instead of overwriting the last one. Pinned saves are always kept, no matter the limits.
luducard-retention-full = Complete copies to keep
luducard-retention-full-desc = Each one holds every save file for the game. More copies means more protection and more disk used.
luducard-retention-differential = Change-only backups per copy
luducard-retention-differential-desc = These store just what changed since the last complete copy, so they are much smaller. Restore points total complete copies × (1 + this number).
