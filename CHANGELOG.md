...
- **"Backup Completed" Even When Nothing Was Saved**: Ludusavi keeps the existing version when a game's saves are identical to the last backup, rather than storing a duplicate. The app reported success either way, so backing up twice looked like a version had gone missing. It now says when there were no changes, and only labels a version it actually created.
...
- **Continuous Integration Failing on Untouched Code**: The workflows tracked whichever Rust release happened to be current, so clippy 1.97 — which learned to look inside `log::` macros — turned the build red across dozens of files nobody had edited, alongside two pre-existing lint errors in the Tauri crate that had been failing all along. All of them are fixed, and the toolchain is now pinned so a compiler release cannot break the build on its own schedule.
- **Test Suite Failing on 0.2.2**: A translation test switched the process-wide language while other tests were asserting against English output, and Rust runs tests in parallel — so seven unrelated report tests failed. The language checks now read the translation files directly instead of mutating shared state, and cover more than before: every one of the 24 shipped files is parsed for syntax errors, and each translated notification is checked for the same variables as the English original.
